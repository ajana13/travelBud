...
import { selectNextQuestion } from "./learning-service.ts";
...
interface ActionResult {
  ...
  eligibleFollowUp: Record<string, unknown> | null;
  feedStale: boolean;
}
...
export async function processAction(
  ...
  let eligibleFollowUp: Record<string, unknown> | null = null;
  if (isNegative) {
    try {
      const learningResult = await selectNextQuestion(userId);
      if (learningResult.prompt) {
        eligibleFollowUp = learningResult.prompt as unknown as Record<string, unknown>;
      }
    } catch {
      // Follow-up is best-effort; swallow errors
    }
  }
